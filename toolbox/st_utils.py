...
def show_logo(st_asset = st.sidebar, use_column_width = False, width = 100, str_color = "purple"):
    ...
    with st_asset:
        ...
        if st.checkbox('pick start date'):
            ...
            tenor = st.text_input('Period', value = default_tenor,
                        help = f'anything that [businessdate](https://businessdate.readthedocs.io/en/latest/intro.html) can understand')
        ...
        interval = st.selectbox('interval', options = l_interval_options,
                        index = l_interval_options.index(default_interval) if default_interval else 0)
